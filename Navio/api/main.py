...
async def lifespan(app: FastAPI):
    ...
    skills_dir = os.getenv("NAVIO_SKILLS_DIR", str(pathlib.Path(_ROOT) / "skills"))
    _skill_manager = SkillManager(
        root_dir=skills_dir,
        max_prompt_chars=int(os.getenv("NAVIO_SKILLS_MAX_PROMPT_CHARS", "5000")),
    )
    _skill_manager.load()
    ...
    logger.info("Navio 已就绪")
    yield

    await _monitor.stop()
    if _memory is not None:
        await _memory.close()
    logger.info("Navio 已关闭")
...
async def _cli():
    ...
    skill_manager = SkillManager(
        root_dir=os.getenv("NAVIO_SKILLS_DIR", str(pathlib.Path(_ROOT) / "skills")),
        max_prompt_chars=int(os.getenv("NAVIO_SKILLS_MAX_PROMPT_CHARS", "5000")),
    )
    skill_manager.load()
    ...
    while True:
        ...
        print(f"\nNavio [{result.agent_type.value}]: {result.response}\n")
