...
本文档说明 Navio 的部署、启动、API 调用、知识库使用、ChromaDB 数据查看、监控评测和常见排障。

Navio 是一个企业级智能客服系统，核心链路为：
...
Navio 常用两种 Docker 启动方式：`docker compose up` 全栈部署，以及 `docker run` 开发模式。两者最大的区别是：**全栈部署会同时启动应用和依赖服务；run 开发模式通常只手动运行一个应用容器，依赖服务需要提前启动**。
...
```bash
docker compose logs -f navio
```

看到 Navio 启动日志并且健康检查通过后，服务可用。
...
Navio 基于 FastAPI 构建，启动 HTTP 服务后可以直接在浏览器访问 Swagger UI 调用接口。
...
Navio 支持从目录加载 Skills，用来把业务流程、客服话术、排障 SOP 等规则动态注入 Agent。

默认配置：

```env
NAVIO_SKILLS_DIR=./skills
NAVIO_SKILLS_MAX_PROMPT_CHARS=5000
```
...
Navio 的知识库由 `mcp/knowledge_base.py` 管理，底层使用 ChromaDB collection：
...
Navio 使用了三个 ChromaDB collection：
...
```bash
docker exec -it navio-chromadb curl http://localhost:8000/api/v1/heartbeat
```
...
docker volume inspect navio_chromadb-data
```

查看容器内数据目录：

```bash
docker exec -it navio-chromadb sh
...
```bash
docker compose down
docker volume rm navio_chromadb-data
docker compose up -d --build
```
...
```bash
docker exec -it navio-redis redis-cli -a navio123
```
...
```bash
docker exec -it navio-redis redis-cli -a navio123 ping
```
...
2. 等待几秒
3. 查看 `docker compose logs -f navio` 是否出现 `用户画像已更新`
